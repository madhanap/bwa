							...
							int state, int is_diff, const gap_opt_t *opt)
{
	...
	if (q->n_entries == q->m_entries) {
		q->m_entries = q->m_entries? q->m_entries<<1 : 2048;
...
void bwt_match_gap1(bwt_t *const bwt, int len, const ubyte_t *seq, bwt_width_t *width, bwt_width_t *seed_width,
						  const gap_opt_t *opt, bwt_aln1_t **_aln, int *_n_aln, int *_act_n_aln, gap_stack_t *stack)
{
   ...
   if (*_act_n_aln < 32) {
	...
	while (stack->n_entries) {
		...
		//fprintf(stderr,"#1\t[%d,%d,%c]\t[%d,%d,%d]\t[%u,%lu]\t[%lu,%lu]\t%d\n", stack->n_entries, i, "MID"[e.state], e.n_mm, e.n_gapo, e.n_gape, width[i-1].bid, width[i-1].w, k, l, e.last_diff_pos);
		...
		if (hit_found) { // action for found hits
			...
			int do_add = 1;
			//fprintf(stderr,"#2 hits found: %d:(%lu,%lu)\n", e.n_mm+e.n_gapo, k, l);
			if (n_aln == 0) {
				best_score = score;
