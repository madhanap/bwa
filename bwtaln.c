...
//if (i % opt->n_threads != tid) continue;
      ...
      bwt_match_gap1(bwt, p->len, p->seq, w, p->len <= opt->seed_len? 0 : seed_w, &local_opt, &p->aln, &p->n_aln, &p->act_n_aln, stack);
...
volatile int count_i;
volatile bool proceed;
volatile bool term[100];
...
void bwa_cal_sa_reg_gap1(const char *prefix, pthread_barrier_t *barrier, int tid, int *n_seqs, bwa_seq_t **seqs, const char *fn_fa, const gap_opt_t *opt)
{
   ...
   n_seqs[tid] = bwa_read_seq1(ks, iter_count, tid, opt->n_threads, &seqs[tid], &size_seqs, opt->mode, opt->trim_qual);
...
++local_iter;
      ...
      if (!tid) {
         ...
         // first thread writes all the output to maintain order
   		for (i = 0; i < opt->n_threads; ++i, ++count_i) {
